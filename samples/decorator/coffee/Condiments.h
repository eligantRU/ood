...
protected:
	...
	std::string GetCondimentDescription() const override
	{
		return std::string((m_liquorType == LiquorType::Cholocate) ? "Chocolate" : "Nuts") + " liquor";
	}
...
public:
		...
		, m_count(count)
	{
		if (m_count > 5)
		{
			throw std::out_of_range("No more than 5 chocolate items as a condiment");
		}
	}
...
protected:
	...
	std::string GetCondimentDescription() const override
	{
		return "Chocolate " + std::to_string(m_count) + "(items)";
	}
